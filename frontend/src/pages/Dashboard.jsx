...
const HIDDEN_EVENT_TYPES = new Set([
  'status', 'artifact', 'agent_typing', 'section_updated', 'section_update',
  'blueprint_update', 'session_created', 'session_loaded', 'impact_assessment',
  'round_started', 'debate_needs_more',
]);

const STREAM_FLUSH_INTERVAL_MS = 400;

const STRUCTURED_DISPLAY_EVENT_TYPES = new Set([
  'agent_selection', 'coordinator_routing', 'session_saved',
]);
...
function isUserFacingEvent(event) {
  ...
  if (event.type === 'phase') return Boolean(content);
  ...
  if (event.type === 'info' && /^(Starting|Product Manager defining|Genesis is convening)/i.test(content)) return false;
...
function eventIdentity(event) {
  if (event?.type === 'user_input' && event?.client_message_id) return `client:${event.client_message_id}`;
  if (event?.id) return `id:${event.id}`;
  if (event?.sequence !== undefined && event?.sequence !== null) return `sequence:${event.run_id || event.chat_id || 'run'}:${event.sequence}`;
  if (event?.streamingKey) return `stream:${event.streamingKey}`;
  if (event?.type === 'user_input') return `user:${eventContentSignature(event.content)}`;
  return ['fallback', event?.type || 'event', event?.agent || '', event?.phase || '', event?.timestamp || '', eventContentSignature(event?.content)].join(':');
}
...
export default function Dashboard({ initialChatId = null, theme = 'light', onToggleTheme }) {
  ...
  const clearConversationState = ({ updateUrl = true, replaceUrl = false } = {}) => {
    ...
    if (updateUrl) navigateChat(null, { replace: replaceUrl });
  };
  ...
  const handleSelectSession = async (chatId, { updateUrl = true, replaceUrl = false } = {}) => {
    ...
    try {
      ...
      setIdea('');
      if (updateUrl) navigateChat(chatId, { replace: replaceUrl });

      const hydratedEvents = dedupeEvents(hydrateSessionEvents(details).filter(isUserFacingEvent));
      setEvents(
        hydratedEvents.length > 0
          ? hydratedEvents
          : [{ type: 'info', content: `Loaded blueprint session for: "${details.user_idea || details.title || chatId}"`, timestamp: details.updated_at }]
      );
      ...
      setEvents((prev) => dedupeEvents([
        ...prev,
        { type: 'error', content: `Failed to load session details: ${error.message}`, timestamp: Date.now() / 1000 },
      ]));
      return false;
    }
  };
  ...
  const handleSelectSessionFromDrawer = async (chatId) => {
    ...
    if (didLoad) setIsSessionDrawerOpen(false);
  };
  ...
  const handleCancelDeleteSession = () => setSessionPendingDelete(null);
  ...
  const handleConfirmDeleteSession = async () => {
    ...
    try {
      ...
      if (currentChatId === chatId) clearConversationState({ updateUrl: true, replaceUrl: true });
    } catch (error) {
  ...
  const handleGenerate = async () => {
    ...
    activeStreamRef.current = { controller, runId, chatId: startingChatId, lastSequence: 0, lastEventId: null, retryCount: 0 };
    ...
    await streamSimulation({
      ...
      onCursor: ({ runId: cursorRunId, lastSequence, lastEventId, retryCount }) => {
        ...
        activeStreamRef.current = { ...(activeStreamRef.current || {}), controller, runId: cursorRunId || runId, lastSequence, lastEventId, retryCount };
      },
      onRetry: ({ retryCount }) => {
        if (activeRunRef.current !== runId) return;
        ...
        setEvents((prev) => dedupeEvents([
          ...prev,
          { id: `reconnect:${runId}:${retryCount}`, type: 'info', agent: 'Genesis', content: 'Connection dipped. Reconnecting to the live run...', timestamp: Date.now() / 1000 },
        ]));
      },
      onEvent: (event) => {
        if (activeRunRef.current !== runId) return;
        ...
        if (event.type === 'session_created' && event.chat_id) {
          activeStreamRef.current = { ...activeStreamRef.current, chatId: event.chat_id };
        ...
        if (event.type === 'phase' && event.content) setCurrentPhase(event.content);
        if (event.agent) setActiveAgent(event.agent);
        if (event.type === 'section_updated' && event.section) applyBlueprintSectionEvent(event);
        if (event.type === 'agent_typing') return;
        ...
        if (event.type === 'session_saved' && event.chat_id) fetchUpdatedSession(event.chat_id, { hydrateEvents: true });
      },
      onError: async (err) => {
        if (activeRunRef.current !== runId) return;
        clearStreamingBuffers();
        ...
        setEvents((prev) => dedupeEvents([...prev, { type: 'error', content: `Simulation failed: ${err.message}`, timestamp: Date.now() / 1000 }]));
        setConnectionState('request-failed');
  ...
  const fetchUpdatedSession = async (chatId, { hydrateEvents = false } = {}) => {
    try {
      ...
      if (hydrateEvents) {
        const hydratedEvents = dedupeEvents(hydrateSessionEvents(details).filter(isUserFacingEvent));
        if (hydratedEvents.length > 0) setEvents(hydratedEvents);
      }
    } catch (e) {
      console.error('Error fetching completed session details:', e);
      if (isSessionNotFoundError(e)) recoverFromMissingSession();
    }
  };
  ...
  const applyBlueprintSectionEvent = (event) => {
    setSessionDetails((prev) => {
      ...
      const sections = mergeBlueprintSection(prev?.sections || {}, event.section, event.after || { content: event.content || '' });
      return { ...(prev || {}), id: prev?.id || chatId, chat_id: chatId, title: prev?.title || idea || 'Live blueprint', user_idea: prev?.user_idea || idea, sections };
    });
  };
  ...
  const bufferStreamingAgentEvent = (event) => {
    ...
    streamingBuffersRef.current.set(key, {
      ...(previous || {}), ...event, id: event.id || previous?.id || `stream:${key}`, type: 'agent_delta', content: bufferedContent, streamingKey: key,
    });
  ...
  const flushStreamingBuffers = () => {
    clearStreamingFlushTimer();
    const bufferedEvents = Array.from(streamingBuffersRef.current.values()).filter((event) => String(event.content || '').trim());
    if (bufferedEvents.length === 0) return;
    setEvents((prev) => {
      const next = [...prev];
      bufferedEvents.forEach((event) => {
        const index = next.findIndex((item) => item.streamingKey === event.streamingKey);
        if (index < 0) { next.push(event); return; }
  ...
  const finalizeStreamingAgentEvent = (prev, event) => {
    const key = streamingKey(event);
    const finalEvent = { ...event, id: event.id || `final:${key}`, streamingKey: key };
  ...
  const handleReset = () => clearConversationState({ updateUrl: true, replaceUrl: false });
  const applyCategoryTemplate = (category) => setIdea(`I want to build a ${category} for [target users] that solves [problem].`);
  ...
  return (
    <div className="genesis-app">
      ...
      <div className="antigravity-overlay">
        <ShapeGrid direction="diagonal" speed={0.5} borderColor={theme === 'dark' ? 'rgba(139, 188, 255, 0.12)' : 'rgba(66, 133, 244, 0.15)'} fadeColor={theme === 'dark' ? 'rgba(16, 20, 26, 0.92)' : 'rgba(255, 255, 255, 1)'} hoverFillColor={theme === 'dark' ? 'rgba(139, 188, 255, 0.08)' : 'rgba(66, 133, 244, 0.08)'} squareSize={48} shape="circle" hoverTrailAmount={6} />
      </div>
      ...
      <main className={`chat-shell ${hasConversation ? 'has-conversation' : 'is-empty'}`}>
        ...
        {connectionState !== 'connected' && (
          <StarBorder className="connection-banner" role="status">
            ...
            <button type="button" onClick={initializeConnection}>Retry</button>
          </StarBorder>
        )}
      ...
      {sessionPendingDelete && (
        <div className="dialog-backdrop" role="presentation">
          <section className="confirm-dialog" role="dialog" aria-modal="true" aria-labelledby="delete-session-title">
            <button type="button" className="dialog-close" onClick={handleCancelDeleteSession} aria-label="Cancel deletion"><X size={17} /></button>
            <div className="dialog-icon danger" aria-hidden="true"><Trash2 size={18} /></div>
            <h2 id="delete-session-title">Delete saved session?</h2>
            <div className="dialog-actions">
              <button type="button" className="dialog-secondary" onClick={handleCancelDeleteSession}>Cancel</button>
              <button type="button" className="dialog-danger" onClick={handleConfirmDeleteSession}>Delete</button>
            </div>
          </section>
        </div>
      )}
